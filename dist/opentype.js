var opentype = (() => {
  ...
  var VariationProcessor = class {
    ...
    applyTupleVariationStore(variationData, points, coords, flavor = "gvar", args = {}) {
      ...
      if (flavor === "gvar" && isCompositeGlyph && compositeGvarType === "rebuild") {
        ...
        const getTopCenterX = (points2) => {
          const ys = points2.map((p) => p.y);
          const maxY = Math.max(...ys);
          const threshold = maxY * 0.7;
          const topPoints = points2.filter((p) => p.y >= threshold);
    ...
    getTransformedPhantomPoints(glyph, variationData, coords) {
      ...
      const leftSideBearing = glyph._originalLeftSideBearing !== void 0 ? glyph._originalLeftSideBearing : glyph.leftSideBearing;
      const advanceWidth = glyph._originalAdvanceWidth !== void 0 ? glyph._originalAdvanceWidth : glyph.advanceWidth;
      const phantomPoints = [
          ...
          if (phantomPoints) {
            ...
            if (isComposite && phantomHasValidVariation && outlineMaxX !== null) {
              if (phantomAdvanceWidth < outlineMaxX) {
                ...
                const originalOutlineWidth = (glyph._originalAdvanceWidth || glyph.advanceWidth) - (glyph._originalLeftSideBearing || glyph.leftSideBearing || 0);
            ...
            if (isComposite && !phantomHasValidVariation) {
              ...
              if (metricsComponent) {
                ...
                if (origCompWidth > 0) {
                  ...
                  const outlineWidth = outlineMaxX - outlineMinX;
                  const originalRSB = (glyph._originalAdvanceWidth || glyph.advanceWidth) - (glyph._originalLeftSideBearing || 0) - originalOutlineWidth;
  ...
  // src/features/latn/contextCheck/ligatureSequence.mjs
  function ligatureSequenceStartCheck(contextParams) {
    const char = contextParams.current;
    const prevChar = contextParams.get(-1);
    return prevChar === null && !isWhiteSpace(char) || isWhiteSpace(prevChar) && !isWhiteSpace(char);
  }
  function ligatureSequenceEndCheck(contextParams) {
    const nextChar = contextParams.get(1);
    return nextChar === null || isWhiteSpace(nextChar);
  }
  var ligatureSequence_default = {
  ...
  Bidi.prototype.contextChecks = {
    ...
    ligatureSequenceCheck: ligatureSequence_default,
  ...
  Font.prototype.forEachGlyph = function(text, x, y, fontSize, options, callback) {
    ...
    for (let i = 0; i < glyphs.length; i += 1) {
      const glyph = glyphs[i];
      const transformedGlyph = this.variation ? this.variation.getTransform(glyph, options.variation) : glyph;
